...
### Fixed
- Fixed critical bug where `git rebase -p` was used instead of `--rebase-merges` (which breaks on Git >= 2.34).
- Fixed issue where `merge` and `tag` messages could not be edited interactively.
- Made remote names dynamic by reading `gitflow.origin` instead of hardcoding `origin`.
