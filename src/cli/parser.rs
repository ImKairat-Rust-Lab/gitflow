...
use crate::{cli::{
    BugfixAction, Cli, Execute, FeatureAction, GitFlowCommands, HotfixAction, InitArgs, ReleaseAction, SupportAction
}, error::AppError};
...
impl Execute for GitFlowCommands {
    fn execute(self) -> Result<(), AppError> {
        match self {
            ...
            Self::Feature { action } => {Ok(())},
            Self::Release { action } => {Ok(())},
            Self::Hotfix { action } => {Ok(())},
            Self::Support { action } => {Ok(())},
            Self::Bugfix { action } => {Ok(())},
